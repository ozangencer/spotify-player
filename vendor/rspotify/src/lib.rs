...
#[inline]
/// Joins the Spotify URIs of the given ids with commas (for the generic `/me/library` endpoints).
pub(crate) fn join_uris<'a, T: Id + 'a>(ids: impl IntoIterator<Item = T>) -> String {
    ids.into_iter().map(|id| id.uri()).collect::<Vec<_>>().join(",")
}
